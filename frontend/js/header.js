document.addEventListener("DOMContentLoaded", async () => {
  ...
  document.querySelectorAll('.nav-link').forEach(link => {
    ...
    if (isLoggedIn && (href && (href.includes('register.html') || href.includes('login.html')))) {
  ...
  function renderAccountUI() { const userContainer = document.querySelector('.user-dropdown-container');
    ...
    userContainer.innerHTML = `
      ...
      <div class="dropdown-menu" id="accountDropdown">
        <a href="profile.html"><i class="far fa-user"></i> Manage My Account</a>
          ...
          if (data.user && data.user.name) {
            ...
            if (data.user.role && data.user.role.toLowerCase() === 'admin_disabled') {
              ...
              accountLinks.forEach(link => {
                  link.href = 'index.html'; // Admin panel is deployed separately
              });
            }
          }
        }
